    ...
    <section class="card" style="margin-bottom: 16px;" aria-label=${u("tabs.work")}>
      ...
      <div style="display: flex; flex-wrap: wrap; gap: 16px; margin-bottom: 12px;">
        <div>
          ...
          ${(()=>{const U=P&&P.length>0?P:zf.map(H=>({value:H.value,label:u(H.labelKey)}));return d`<select
              .value=${p}
    ...
    ${r==="awaiting_choices"&&a.length?(()=>{const U=r;return d`
            <section class="card" style="margin-bottom: 16px;" aria-live="polite">
              ...
              <div style="display: flex; gap: 8px; margin-top: 12px;">
                ...
                ${U==="error"?d`<button class="btn btn-sm" @click=${j}>${u("common.retry")}</button>`:$}
              </div>
            </section>
          `})():$}
...
${O.name??""}
${O.source??""}`.toLowerCase().includes(D)):i].sort((O,ne)=>_v(O,ne,c,f)),C=_.length,z=Math.max(1,g||10),I=Math.max(1,Math.ceil(C/z)),j=Math.min(Math.max(1,p),I),K=(j-1)*z,G=_.slice(K,K+z);return d`
...
${L.specification??""}
${L.code??""}
${L.product_key??""}`.toLowerCase().includes(y)):D,C=[..._].sort((L,Y)=>Cv(L,Y,c,f)),z=C.length,I=Math.max(1,g||10),j=Math.max(1,Math.ceil(z/I)),K=Math.min(Math.max(1,p),j),G=(K-1)*I,O=C.slice(G,G+I),ne=_.filter(L=>o.includes(De(L))).length,be=_.length>0&&_.every(L=>o.includes(De(L)));return d`
...
//# sourceMappingURL=index-DpPk2bDm.js.map
