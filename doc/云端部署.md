...
- **启动命令规范化**：固定一条「云端启动命令」（例如 `python run_backend.py`，见下 Render 说明），并在 README/部署脚本中统一。

**Render 部署注意（Port scan timeout 排查）**：若出现 “Port scan timeout reached, no open ports detected”，多为**启动命令错误**。Render 要求主进程**常驻前台**并监听 `0.0.0.0:$PORT`。本仓库已满足：`Config.API_HOST=0.0.0.0`、`Config.API_PORT` 取自环境变量 `PORT`。请勿使用 `python start.py`（start.py 会 spawn 子进程后自己退出，Render 会判为主进程退出、部署失败）。正确做法：**Start Command 填 `python run_backend.py`**。若使用 Blueprint，仓库根目录已提供 `render.yaml`，其中 `startCommand` 已设为 `python run_backend.py`。
- **Docker（可选但推荐）**：  
