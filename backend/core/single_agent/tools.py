...
def _get_inventory_tools():
    from inventory_agent.services.inventory_agent_tools import get_inventory_tools_openai_format
    return get_inventory_tools_openai_format()


def _get_quote_tools():
    from backend.agents.quote.quote_tools import get_quote_tools_openai_format
...
def _run_oos_list(limit: int = 100) -> dict:
    try:
        from quotation_tracker.services.data_service import DataService
        ds = DataService()
...
def _run_register_oos(file_path: str, context: Optional[Dict] = None, prompt: Optional[str] = None) -> dict:
    try:
        from quotation_tracker.services.agent_runner import run_quotation_agent
...
async def execute_tool(
    ...
    if name in _INVENTORY_TOOLS:
        try:
            from inventory_agent.services.inventory_agent_tools import execute_inventory_tool
