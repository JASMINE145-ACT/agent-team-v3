...
def record_correction_to_knowledge(
    keywords: str,
    confirmed_code: str,
    confirmed_name: str,
    reasoning: str = "",
    ...
    """
    将用户在 rework 流程中确认的正确选项追加到 wanding_business_knowledge.md。
    追加格式为「## 用户纠正案例」，使 LLM 下次遇到相似询价时能直接选对。
    返回成功或错误文案。
    """
    path = _get_knowledge_path()
    if not path:
    ...
    try:
        ...
        reason_text = f"原因：{reasoning}" if reasoning else ""
        entry = f"""
## 用户纠正案例
- 询价：「{keywords}」
  → 正确选择：[{confirmed_code}] {confirmed_name}
  {reason_text}
  时间：{timestamp}
"""
        with path.open("a", encoding="utf-8") as f:
        ...
        return (
            f"已记录纠正案例：询价「{keywords}」→「{confirmed_code} {confirmed_name}」。"
            "下次遇到相似询价时将优先参考此选择。"
        )
    except Exception as e:
        return f"记录纠正案例失败：{e}"
