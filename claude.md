...
- `POST /api/query` 或 `POST /api/master/query`：Body `{ "query": "用户输入", "session_id": "可选", "context": { "file_path": "可选" } }`，返回 `answer`、`trace`、`thinking` 等。Chat 入口会做轻量语言检测：英文问题会在 `context.preferred_lang=\"en\"` 下进入 ReAct 循环，`JAgentExtension` 会在 prompt 中注入「必须全英文回答」策略，并在工具 observation 为中文时附加英文翻译提示；中文问题保持 `preferred_lang=\"zh\"`，行为与之前一致。
